...
type iterator = {
  ...
  signature_item_extension: iterator -> Extensions.Signature_item.t -> unit;
  structure: iterator -> structure -> unit;
  structure_item: iterator -> structure_item -> unit;
  structure_item_extension: iterator -> Extensions.Structure_item.t -> unit;
...
module MT = struct
  ...
  let iter_signature_item_ext sub : Extensions.Signature_item.t -> unit =
    function
    | Esig_include_functor ifincl -> iter_sig_include_functor sub ifincl

  let iter_signature_item sub ({psig_desc = desc; psig_loc = loc} as sigi) =
    sub.location sub loc;
    match Extensions.Signature_item.of_ast sigi with
    | Some esigi -> sub.signature_item_extension sub esigi
    | None ->
    match desc with
...
module M = struct
  ...
  let iter_structure_item_ext sub : Extensions.Structure_item.t -> unit =
    function
    | Estr_include_functor ifincl -> iter_str_include_functor sub ifincl

  let iter_structure_item sub ({pstr_loc = loc; pstr_desc = desc} as stri) =
    sub.location sub loc;
    match Extensions.Structure_item.of_ast stri with
    | Some estri -> sub.structure_item_extension sub estri
    | None ->
    match desc with
    | Pstr_eval (x, attrs) ->
...
let default_iterator =
  {
    ...
    structure_item_extension = M.iter_structure_item_ext;
    module_expr = M.iter;
    signature = (fun this l -> List.iter (this.signature_item this) l);
    signature_item = MT.iter_signature_item;
    signature_item_extension = MT.iter_signature_item_ext;
    module_type = MT.iter;
