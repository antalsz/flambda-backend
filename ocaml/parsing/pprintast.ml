...
and signature_item_extension ctxt f : Extensions.Signature_item.t -> _ =
  function
  | Esig_include_functor ifincl -> sig_include_functor ctxt f ifincl

and signature_item ctxt f x : unit =
  match Extensions.Signature_item.of_ast x with
  | Some esigi -> signature_item_extension ctxt f esigi
  | None ->
...
and str_include_functor ctxt f
  : Extensions.Include_functor.structure_item -> _ = function
  | Ifstr_include_functor incl ->
      include_ ctxt f ~functor_:true ~contents:module_expr incl

and structure_item_extension ctxt f : Extensions.Structure_item.t -> _ =
  function
  | Estr_include_functor ifincl -> str_include_functor ctxt f ifincl

and structure_item ctxt f x =
  match Extensions.Structure_item.of_ast x with
  | Some estri -> structure_item_extension ctxt f estri
  | None ->
