...
and approx_include_functor
      env (ifincl : Extensions.Include_functor.signature_item) _srem =
  match ifincl with
  | Ifsig_include_functor sincl ->
      raise (Error(sincl.pincl_loc, env, Recursive_include_functor))

and approx_sig_ext' env (eitem : Extensions.Signature_item.t) srem =
  match eitem with
  | Esig_include_functor ifincl -> approx_include_functor env ifincl srem

and approx_sig env ssg =
  match ssg with
    [] -> []
  | item :: srem ->
      match Extensions.Signature_item.of_ast item with
      | Some eitem -> approx_sig_ext' env eitem srem
      | None ->
...
and transl_signature env (sg : Parsetree.signature) =
  ...
  let transl_include_functor ~loc env sig_acc
    : Extensions.Include_functor.signature_item -> _ = function
    | Ifsig_include_functor sincl ->
        transl_include ~functor_:true ~loc env sig_acc sincl
  in

  let transl_sig_item_ext ~loc env sig_acc : Extensions.Signature_item.t -> _ =
    function
  ...
  let transl_sig_item env sig_acc item =
    ...
    match Extensions.Signature_item.of_ast item with
    | Some eitem -> transl_sig_item_ext ~loc env sig_acc eitem
    | None ->
    match item.psig_desc with
    | Psig_value sdesc ->
        let (tdesc, newenv) =
...
and type_structure ?(toplevel = None) funct_body anchor env sstr =
  ...
  let type_str_include_functor ~loc env shape_map ifincl sig_acc =
    match (ifincl : Extensions.Include_functor.structure_item) with
    | Ifstr_include_functor incl ->
        type_str_include ~functor_:true ~loc env shape_map incl sig_acc
  in

  let type_str_item_ext ~loc env shape_map eitem sig_acc =
    match (eitem : Extensions.Structure_item.t) with
  ...
  let type_str_item
    ...
    match Extensions.Structure_item.of_ast item with
    | Some eitem -> type_str_item_ext ~loc env shape_map eitem sig_acc
    | None ->
    match desc with
